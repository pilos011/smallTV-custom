...
public:
  ...
  // Vendored change: the workspace lives on the heap for the milliseconds a
  // decode actually runs, instead of 3.5 KB of static RAM standing for ever
  // on a chip with 80 KB. new[] guarantees the 32-bit alignment jd_prepare
  // needs. Allocated and released by each public draw/size entry point.
