...
**format 은 응답을 먼저 보내고, 지운 뒤 설정을 되쓰고, 재시작합니다(v1.0.30~).** 세 가지가
다 이유가 있습니다.

- **설정은 RAM 에 그대로 있습니다.** 지우기가 건드리지 않으므로 새 볼륨에 다시 씁니다.
  format 이 잃는 것은 사진과 웹 파일뿐입니다 — 되쓰지 않으면 마운트를 고치러 간 라우트가
  그 기기가 다시 접속할 WiFi 자격증명까지 가져갑니다
- **재시작합니다.** 소켓을 닫고 `handleClient()` 로 돌아가면 다른 어떤 라우트도 두지 않는
  상태가 됩니다. 볼륨이 발밑에서 사라진 동안 파일을 잡고 있던 것들도 재시작이 정리합니다
- **`PH_FORMAT` 단계를 먼저 남깁니다.** 2MB 지우기는 8초 하드웨어 워치독보다 오래 걸리고
  소프트웨어로 먹일 수 없어, 성공한 format 도 다음 부팅에 `REASON_WDT_RST` 로 도착합니다.
  단계를 안 남기면 **시키는 대로 한 죄로 세 번이면 safe mode** 입니다. 그 워치독이 물면
  설정 되쓰기는 실행되지 않으므로, 응답 문구도 약속하지 않고 확인 방법을 안내합니다

**읽히지 않는 `config.json` 은 `/config.bad.json` 으로 옮깁니다(v1.0.30~).** 펌웨어가 못
읽는 것과 사람이 못 읽는 것은 다릅니다 — API 키와 WiFi 비밀번호가 평문으로 그 안에 있고
`/file?path=/config.bad.json` 으로 꺼낼 수 있습니다. 옮기지 않으면 다음 크래시 후 첫 부팅이
카운터를 쓰면서 파일 전체를 가져갑니다.
...
- **저장을 막는 조건은 safe mode 가 아니라 `configLoaded` 다(v1.0.30~).** `!bootSafeMode`
  ...
  이른 진입은 `loadConfig` 전이라 `configLoaded` 가 false 고 기본값으로 덮는 것을 막습니다.
  늦은 진입은 true 라 쓸 수 있고, 그 파일을 쓰는 것이 빠져나오는 유일한 길입니다
- **파일 쓰기는 실패하면 되돌리고 다시 시도한다(v1.0.30~).** `bootFailsPersist()`. 값을
  먼저 대입하면 저장 실패가 자기 가드를 스스로 해소해서 아무도 다시 시도하지 않습니다.
  1분 간격 5회까지 — 시도마다 임시 파일을 쓰고 지우므로 무한이면 플래시를 갉습니다
- **파일 쓰기는 `setupNetwork()` 뒤다.** 라우트 뒤로만 옮기는 것은 절반입니다. 인터페이스가
  없으면 핸들러가 등록돼 있어도 닿을 수 없습니다
...
2. 소스의 `FW_VERSION`, 문서의 버전 표기, 그리고 **`data/web/index.html`의 `?v=`**를 새
   버전으로 맞춥니다. 세 번째는 잊기 쉬운데, 틀려도 아무 증상이 없습니다 — 페이지는 뜨고
   옛 스크립트가 돌아서 방금 올린 수정이 안 먹은 것처럼 보입니다. `build_release.ps1`이
   파일시스템 이미지를 만들기 **전에** 대조하고 어긋나면 중단합니다.
...
- **순정 상태의 새 장치는 아래 순서로 이관합니다. UART는 필요 없습니다** —
  2026-09-02에 네 대를 이 순서로, 전부 OTA만으로 옮겼습니다. 자세한 것은
  `docs/STOCK_MIGRATION.md`.
