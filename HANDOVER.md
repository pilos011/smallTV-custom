...
| 파일 | 옮기는 것 | 개수 |
| --- | --- | --- |
| `display/Airlines.h` | 호출부호 세 글자 → 항공사명 | 49 |
| `display/Rotorcraft.h` | ICAO 기종 코드 → 기체 이름 | 43 |
| `display/Airports.h` | 목적지 IATA → 공항·도시명 | 155 |

셋 다 생성물이고, 표에 없으면 원래 문자열을 그대로 씁니다 — 틀린 이름보다
코드가 낫습니다. 회전익은 ADS-B 카테고리 `A7`로 가려내 다른 표식으로 그리고,
항공사 자리에 기종 이름을 넣습니다.
