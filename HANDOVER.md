...
비 오는 줄은 **우산**을 그립니다(v1.0.25~). 28px 비 아이콘은 구름에 가느다란 파란 획
세 개뿐이라 옆줄의 흐림과 구분되지 않았습니다. `forecastIcon()` 이 이 화면에서만
`rain` 을 `umbrella` 로 바꾸며, 대시보드는 같은 슬롯을 52px로 그리고 그 크기에서는
빗방울이 보이므로 건드리지 않습니다. `umbrella-28.bmp` 는 원본 54x58에서 줄인 것으로,
없으면 그 줄의 아이콘 자리만 빕니다.
