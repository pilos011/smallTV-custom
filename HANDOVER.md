...
| `radar_routes` | 목적지 조회 사용 여부 | `true` |
...
**항공사와 기종 이름.** `display/Airlines.h`가 호출부호 세 글자를 한글
항공사명으로(49개), `display/Rotorcraft.h`가 ICAO 기종 코드를 기체 이름으로
(43개) 옮깁니다. 둘 다 생성물이며 표에 없으면 원래 문자열을 그대로 씁니다.
회전익은 ADS-B 카테고리 `A7`로 가려내 다른 표식으로 그리고, 항공사 자리에
기종 이름을 넣습니다.

두 표 모두 이름을 **PROGMEM 문자열로 두고 `strncpy_P`로 꺼냅니다.** PROGMEM
